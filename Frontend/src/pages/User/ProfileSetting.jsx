...
export default function ProfileSetting() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get(`${import.meta.env.VITE_API_URL}/api/user/profile`);
  ...
  const handleProfileSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.put(`${import.meta.env.VITE_API_URL}/api/user/update`, {
        name: user.name,
        phone: user.phone,
      });
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center space-y-4">
        <motion.img
          src={preview || user?.avatar || avatarPlaceholder}
          alt="Avatar"
        ...
        <p className="text-md font-bold text-gray-800">{user.channelName}</p>
      ...
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-8 text-sm text-gray-700">
        ...
        >
          ...
          {user.premium === "Free" && (
            <div>
              <Label className="mb-2">Premium Plan</Label>
              <select
                name="premium"
                value={user.premium}
